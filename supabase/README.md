...
| `migrations/20260926093811_merchant_state_foundation.sql` | D1a: canonical merchant state, shared public predicate, audit log, one active owner, private view counts. One transaction, self-checking; existing rows stay `legacy` | Local → staging → production, each after review/approval |
