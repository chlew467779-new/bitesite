...
export default function MerchantDashboardPage() {
  ...
  return <main className="min-h-screen bg-[#FAFBF7] px-4 py-12 sm:py-20"><div className="mx-auto max-w-3xl space-y-6">
    <header className="rounded-2xl border border-[#DDE5DC] bg-white p-6 shadow-sm sm:p-8"><p className="text-sm text-[#6B6560]">Merchant dashboard</p><h1 className="mt-1 font-serif text-3xl text-[#2C3E2D]">{merchant.name}</h1><div className="mt-4 flex flex-wrap gap-x-5 gap-y-2 text-sm text-[#6B6560]"><a className="text-emerald-700 underline" href={`/store/${merchant.slug}`}>View public listing</a><Link href="/merchant/stories" className="text-emerald-700 underline">Submit or manage Stories</Link><span>Listing: {merchant.platform_status || 'PUBLISHED'}</span><span>Business: {merchant.business_status || 'OPEN'}</span></div></header>
    <form onSubmit={save} className="rounded-2xl border border-[#DDE5DC] bg-white p-6 shadow-sm sm:p-8"><div><h2 className="font-serif text-2xl text-[#2C3E2D]">Update your listing</h2><p className="mt-1 text-sm text-[#6B6560]">These details update your public BiteSite page directly.</p></div><div className="mt-6 space-y-5">
      <label className="block text-sm font-medium text-[#2C3E2D]">Tagline<input name="tagline" defaultValue={merchant.tagline || ''} className="mt-1.5 w-full rounded-lg border border-[#DDE5DC] px-3 py-2 text-sm" /></label>
      <label className="block text-sm font-medium text-[#2C3E2D]">About your restaurant<textarea name="description" defaultValue={merchant.description || ''} rows={5} className="mt-1.5 w-full rounded-lg border border-[#DDE5DC] px-3 py-2 text-sm" /></label>
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <label className="text-sm font-medium text-[#2C3E2D]">Phone<input name="phone" defaultValue={merchant.phone || ''} className="mt-1.5 w-full rounded-lg border border-[#DDE5DC] px-3 py-2 text-sm" /></label><label className="text-sm font-medium text-[#2C3E2D]">WhatsApp<input name="whatsapp" defaultValue={merchant.whatsapp || ''} className="mt-1.5 w-full rounded-lg border border-[#DDE5DC] px-3 py-2 text-sm" /></label><label className="text-sm font-medium text-[#2C3E2D]">Email<input name="email" type="email" defaultValue={merchant.email || ''} className="mt-1.5 w-full rounded-lg border border-[#DDE5DC] px-3 py-2 text-sm" /></label><label className="text-sm font-medium text-[#2C3E2D]">Website<input name="website" type="url" defaultValue={merchant.website || ''} className="mt-1.5 w-full rounded-lg border border-[#DDE5DC] px-3 py-2 text-sm" /></label><label className="text-sm font-medium text-[#2C3E2D]">Instagram URL<input name="instagram" type="url" defaultValue={merchant.instagram || ''} className="mt-1.5 w-full rounded-lg border border-[#DDE5DC] px-3 py-2 text-sm" /></label><label className="text-sm font-medium text-[#2C3E2D]">Facebook URL<input name="facebook" type="url" defaultValue={merchant.facebook || ''} className="mt-1.5 w-full rounded-lg border border-[#DDE5DC] px-3 py-2 text-sm" /></label><label className="text-sm font-medium text-[#2C3E2D]">Logo image URL<input name="logo_image" type="url" defaultValue={merchant.logo_image || ''} className="mt-1.5 w-full rounded-lg border border-[#DDE5DC] px-3 py-2 text-sm" /></label><label className="text-sm font-medium text-[#2C3E2D]">Cover image URL<input name="cover_image" type="url" defaultValue={merchant.cover_image || ''} className="mt-1.5 w-full rounded-lg border border-[#DDE5DC] px-3 py-2 text-sm" /></label>
      </div>
      <label className="block text-sm font-medium text-[#2C3E2D]">Menu PDF URL<input name="menu_pdf_url" type="url" defaultValue={merchant.menu_pdf_url || ''} className="mt-1.5 w-full rounded-lg border border-[#DDE5DC] px-3 py-2 text-sm" /></label>
      <fieldset><legend className="text-sm font-medium text-[#2C3E2D]">Opening hours</legend><p className="mt-1 text-xs text-[#6B6560]">For example: 10:00 AM – 10:00 PM. Leave a day blank if closed.</p><div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2">{days.map(day => <label key={day} className="text-sm capitalize text-[#2C3E2D]">{day}<input name={`hours-${day}`} defaultValue={merchant.operating_hours?.[day] || ''} className="mt-1 w-full rounded-lg border border-[#DDE5DC] px-3 py-2 text-sm" /></label>)}</div></fieldset>
