...
export default function MerchantStoriesPage() {
  ...
  const formContent = <form onSubmit={submit} className="mt-6 space-y-5 rounded-2xl border border-[#DDE5DC] bg-white p-6">
    <div><label className="block text-sm font-medium text-[#2C3E2D]">Story title<input required maxLength={160} value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} placeholder="Example: A new weekend brunch at The Hearth Bakery" className="mt-1 w-full rounded-lg border border-[#DDE5DC] px-3 py-2 text-sm" /></label><p className="mt-1 text-xs text-[#6B6560]">A clear title helps readers understand the story quickly.</p></div>
    ...
    <div><label className="block text-sm font-medium text-[#2C3E2D]">Short excerpt <span className="font-normal">(optional)</span><input value={form.excerpt} onChange={e => setForm({ ...form, excerpt: e.target.value })} placeholder="One or two sentences for the Story card" className="mt-1 w-full rounded-lg border border-[#DDE5DC] px-3 py-2 text-sm" /></label><p className="mt-1 text-xs text-[#6B6560]">This may appear in previews and search results.</p></div>
    <div><label className="block text-sm font-medium text-[#2C3E2D]">Facts and source material<textarea required maxLength={20000} rows={9} value={form.content} onChange={e => setForm({ ...form, content: e.target.value })} placeholder="Include what happened, what is offered, price, dates, location, and anything readers should know" className="mt-1 w-full rounded-lg border border-[#DDE5DC] px-3 py-2 text-sm" /></label><p className="mt-1 text-xs text-[#6B6560]">Specific facts help our editorial team write accurately. Do not include customer personal data, unsupported health claims, or information you do not have permission to publish.</p></div>
